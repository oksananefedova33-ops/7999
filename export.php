...
function exportSite() {
    try {
        ...
        // Генерируем токен для удаленного управления
$remoteToken = bin2hex(random_bytes(32));

// Создаем файл remote-api.php с токеном
$remoteApiContent = file_get_contents(__DIR__ . '/remote-api-template.php');
$remoteApiContent = str_replace('{{GENERATED_TOKEN}}', $remoteToken, $remoteApiContent);
file_put_contents($exportDir . '/remote-api.php', $remoteApiContent);

// Сохраняем токен в файл info.txt для пользователя
$infoContent = "=== ИНФОРМАЦИЯ ОБ ЭКСПОРТИРОВАННОМ САЙТЕ ===\n\n";
$infoContent .= "Дата экспорта: " . date('Y-m-d H:i:s') . "\n";
$infoContent .= "Токен для удаленного управления: " . $remoteToken . "\n\n";
$infoContent .= "Для удаленного управления сайтом:\n";
$infoContent .= "1. Загрузите файлы на хостинг\n";
$infoContent .= "2. В редакторе нажмите 'Мои сайты'\n";
$infoContent .= "3. Добавьте домен сайта и токен\n\n";
$infoContent .= "ВАЖНО: Храните токен в безопасности!\n";
file_put_contents($exportDir . '/REMOTE_ACCESS_INFO.txt', $infoContent);
        ...
        foreach ($pages as $page) {
            ...
            $html = generatePageHTML($pdo, $page, 'ru', null, $usedFiles, $languages);
            ...
            if (!empty($languages) && !empty($translations)) {
                foreach ($languages as $lang) {
                    ...
                    $html = generatePageHTML($pdo, $page, $lang, $pageTrans, $usedFiles, $languages);
...
function getPageFilename($page, $lang = 'ru') {
    ...
    // Добавляем языковой суффикс для всех языков кроме русского
    if ($lang !== 'ru') {
        $basename .= '-' . $lang;
    }
...
function generatePageHTML($pdo, $page, $lang, $translations, &$usedFiles, $allLanguages) {
    ...
    switch ($type) {
        ...
        case 'langbadge':
            ...
            foreach ($langs as $l) {
                ...
                $pageFilename = getPageFilename($page, $l); // напр.: index-en.html
